...
import { EIP712MessageTypesEntry } from "./types/eip712-types";
...
export const EIP712_TYPE_ENCODERS = {
  INT(value: string | null | any, sizeInBits = 256): Buffer {
  ...
  UINT(value: string | any): Buffer {
    return this.INT(value);
  },

  BOOL(value: number | string | boolean | null | any): Buffer {
    return this.INT(typeof value === "boolean" ? Number(value).toString() : value);
  },

  ADDRESS(value: string | null | any): Buffer {
    // Only sending the first 10 bytes (why ?)
    return hexBuffer(value ?? "").slice(0, 20);
  },

  STRING(value: string | null | any): Buffer {
    return Buffer.from(value ?? "", "utf-8");
  },

  BYTES(value: string | null | any, sizeInBits?: number): Buffer {
